...
class TestExecute(TestCase):
    ...
    def test_execute_multiple_non_streaming(self, mock_handler):
        ...
        session_instance.execute.assert_has_calls([call('query1'), call('query2')])

        self.assertEqual(session_instance.execute.return_value, res)

    @patch("gobupload.storage.execute.GOBStorageHandler")
    def test_execute_multiple_streaming(self, mock_handler):
        res = _execute_multiple(['query1', 'query2'], stream=True)
